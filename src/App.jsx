...
function ProgressMonitor({ onDone, delay = 500 }) { // delay in ms
    ...
    <div className="w-screen h-screen">
      <Canvas camera={{ position: [0, 4, 20], fov: 60 }} shadows style={{ backgroundColor: "#222" }}>
        ...
        <Suspense fallback={null}>
          <ProgressMonitor onDone={() => setAssetsLoaded(true)} delay={800} />
          <CameraController
            mode={mode}
            focus={focus}
      ...
      {!assetsLoaded && <LoaderOverlay />}

      {/* Movement hint */}
      {mode !== "in" && !showOverlay && !showStartOverlay && <MovementHint />}
